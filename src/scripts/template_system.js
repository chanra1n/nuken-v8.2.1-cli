...
//same init function as addons and themes, 
// basically we get all the detected .js files from the templates/ directory and add them to the array.
var init_templates = function() {
  window.api.templates().then((result) => {
    for (var i = 0; i < result.length; ++i) {
      ...
      // Use a closure to capture the item variable
      (function(item) {
        setTimeout(function() {
      ...
      })(item); // Immediately invoke the closure with the current item
    }
  });
};
...
//listen for template input, only add templates to the menu when this is true
...
var push_template = function(template) {
	...
	// push an icon to the Template Menu, when clicked it loads in our template object. 
	// we have to pass the object name as a paramter, which is why we need the "codename" property... 
	// it doesn't like the object for some reason, we just need its name
...
`;
	...
	if (template.codename === "welcome_to_nuken") {} else {
...
var select_template = function(template) {
	...
	// this is actually important this time around, 
	// as nuken can load templates from a external location (via a link), or they can be installed on the user's device.
	...
	// if the template specifies a codename for a resource in the Stylesheet or Script Management menu 
	// (if the template doesn't use just plain ol' JS + CSS), load that in here.
...
var write_template = function() {
	...
	// write a template to the workspace, once the boxes are filled with content. 
	// For whatever reason, the actual project preview isn't updated - 
	// unless we manually create a new preview frame (using the same methods as project_preview).
	// the rest of this function is pretty much a cut/paste from that function.
