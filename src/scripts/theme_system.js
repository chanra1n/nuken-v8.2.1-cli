/*
Pretty much we're just appending a custom stylesheet to the document here. 
Nothing special. nuken detects CSS files from the /themes directory.

There's a remove element function we added here too. Idk lol.
...
<i class = "ri-paint-fill content_div_icon"></i> 
	...
	`;
		...
		var theme_panel_raw = `
<div id = "` + filename + `_file" class = "content_div">
<img src = "icons/theme.png" ></img>
<div id = "` + filename + `_file_body" >
<span>` + cosmetic_filename2 + `</span>
<br>
<button onclick = "show_folder('/` + theme + `')" ><i class="ri-external-link-line"></i></button>
<button onclick = "delete_file('/` + theme + `');show_deleted_element('` + filename + `_file')" ><i class="ri-delete-bin-2-line"></i></button>
<i class = "ri-paint-fill content_div_icon"></i> 
</div>
</div> 
	`;
