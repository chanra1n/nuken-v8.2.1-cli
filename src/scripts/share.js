//indicates whether the Share Menu is open or not.
...
var write_preview = function() {
	...
	if (share_menu_is_open) {
		...
		// When the user creates a snapshot and the menu is OPEN, the camera shutter sound plays. 
		// However, since we don't want the shutter sound to play if the user isn't actually in the menu, 
		// we detect whether it's open or not first. (Remember, when the user hits the play button, a snapshot is automatically generated).
	...
	// here, we create a (hidden) iframe based on the project currently being previewed in the preview area
	// (taking a snapshot of the preview area doesn't work - we need to create a new iframe e can manipulate for this to work. trust me bro). 
	// Then, we use html2canvas to take a snapshot of this iframe, and display the result to the user.
