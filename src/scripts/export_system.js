...
// this is the function that's called when the "Export project" button is clicked.
// it essentially tells nuken to download the user's project based on a selected filename,
// and some text (this is written to the file we're downloading).
...
// if the project config menu is set up to look for an absolute file path on the user's system,
// this bit of code is appended to the project to add it to every relative file path. 
...
var init_custom_file_paths = function() {
	...
	// first, we save the values of the html, css, and js code boxes.
	// plus we get what's written to the custom file path input box.
	...
	if (loc == "") {
		...
		//this bit of code looks for every element with a link, src, href, etc. tag - 
		// and appends the LOC variable to the beginning of the relative file path.	
