...
var init_addons = function() {
    ...
    window.api.addons().then((result) => {
        ...
        for (var i = 0; i < result.length; ++i) {
            ...
            //because index.html is located in a different directory than the /addons directory, 
			//we have to go back a bit to find it the actual .js file.
            var item = addon_direc + result[i];

			//push the file to the addon index
            addons_index.push(item); 

            console.log("inited " + item);
			
            // load the add-on into the workspace, more on that later
			load_addon(item); 
...
// indicates whether nuken is ready to write add-on data to the add-ons menu
        ...
        if (content_menu_available) {
			...
			<div id = "` + filename + `" class = "content_div">
				...
				<div id = "` + filename + `_body" >
					...
					<button onclick = "delete_file('/` + url + `');show_deleted_addon('` + filename + `')" ><i class="ri-delete-bin-2-line"></i></button>
...
var push_addon = function(addon) {
    ...
    /*
    nuken displays a "no addons?" message in the add-on menu if no add-ons are detected.
	If add-ons are detected, nuken removes this message and begins to push add-on icons to the dropdown. 
    ...
    Basically, we don't want to wipe the dropdown every single time an add-on is pushed to the menu.
	We want to wipe it the FIRST time, and if nuken isn't ready for add-ons yet. 
	After the "no addons?" menu is wiped, nuken is ready for add-on input, and ready_for_addons is set to true.
	This keeps this bit of code from running over and over again.
    */
    ...
    // The following code checks for the existence of optional properties in the passed object. 
	// If the optional property doesn't exist, nuken fills in a placeholder value. However, some properties are required. 
	// These aren't checked, as they need to exist already to even get this far.

    // you may notice i've used _ in place of . for the property check variables.
	// this is intentional, having new variables we can easily modify the value of sure beats having to
	// re-set the value of a property (that may or may not exist).
	// This would be a headache to figure out (for me). the way i have it is easier i think. source: trust me bro plz
    ...
    if ('pin' in addon) {
        ...
        // If the value of addon.pin = false, then the display property of the "pin" button is set to none.
		// If it's set to true, this is passed to the display property as well, 
		// but since "true" is not a valid property value for display, it's ignored - which is percisely what we want.		
        addon_pin = "none";
    }
    ...
    if ('donate' in addon) {
        ...
        // If this property does not exist in the addon object, "none" is passed in its place.
		// This hides the "donate" button, by setting the display value to "none". 
		// If addon.donate exists, we don't interfere and modify the value.
		// This passes a URL to the display property for the "donate" button, and since it's an invalid value, it's ignored.

        // The donate button is set to open the URL dictated by addon.donate, regardless of its existence or value.
		// If addon.donate doesn't exist, it'll probably open a blank page -
		// but since "none" is passed as the display value when this is the case, you won't see the broken button.
    ...
    if ('link' in addon) {
        ...
        //For all intents and purposes, the link button is a clone of the donate button.
		// Same logic applies as above - basically, if addon.link exists, show the button, if not, hide the broken button.
        addon_link = "none";
    }
    ...
    if ('online' in addon) {
        ...
        // If an Add-on creator doesn't want to spend the time to make an "add-on is offline, please connect to the internet" 
		// screen for when nuken is offline and they need to connect to the internet, we can handle this for them.
		// Basically, if this is set to true, the add-on can only be used online and the related function below is executed.
		// If it doesn't exist, or is set to false, obviously the add-on can be	used offline, and the related function below is ignored.
        addon_online = false;
    }
    ...
    //Right now, shop ids are a bit problematic, as they currently have no value. 
	// We hand them out to people who've listed their content on the nuken Shop,
	// which right now is just first-party content so it doesn't really matter.
	// but right now we don't have a way for that ID to translate to an actual shop listing.
	// However, in the past we've accomplished this just fine, so it's just a matter of implementing it again.
	// But since we haven't handed out any IDs yet, we're fine.
	
	// UPDATE: we're not doing this. we have other ways for creators to direct users to their content.
	// This is junk. Please remove it by v8.3.0.
    ...
    // addon.fullscreen can be set to either true or false, 
	// and if true, this overrides the values of addon.resize, addon.width, and addon.height; in a related function below.
    ...
    // if addon_fullscreen = true, then none of the resize, width, or height values matter.
	// the window takes up all available space.
...
`;
    ...
    // write the actual add-on to its own window.
	// here, we generate a brand new UIkit modal in the div #addon_sandbox, where all the add-on windows are written.
	// All of the attributes we defined above are written or attributed to this new add-on window modal div thingy.
...
`;
    ...
    // if no value was provided for addon.pin, or it was set to false, addon_pin was set to "none".
	// this is reflected here. no pinning the add-on plz.

    if (addon_pin == "none") {
        document.getElementById(addon.codename + "_icon").setAttribute('draggable', false);
    }

    // if the addon creator set their addon.online to true, addon_online will have been set to true.
	// if THIS is the case, if nuken is offline, write the addon_text to the element 
	// "<ADDON CODENAME>_modal_inner" (the inside of the addon window in question). 
	// This wipes everything from the window unfortunately, so we have to rewrite the close button to the window.
...
// UPDATE: yep, it sure doesn't you dumbass. you can remove it next update, mkay?
...
var pin_addon = function(addon) {
    ...
    //if the pin button says "pinned", that means the addon is already pinned.
	// don't pin it again, and don't let the user drag the icon to pin it.
    ...
    if (document.getElementById(addon.codename + "_pin_activate").innerHTML.toString().toLowerCase().includes('pinned')) {
        ...
        // here, we do another series of checks for optional properties a pinned add-on needs.
		// there are explained above in load_addon(), so go read those.
        ...
        // If you open DevTools, you'll see that nuken is split into some organizational divs.
		// this div ('#menus') is where all the menus are written to, and where we'll write our draggable mini pinned addon thingy. 
...
if (files.includes('raw') && files.includes('githubusercontent')){
downloadable = files;
} else {
...
document.getElementById('shop_download_location').style.backgroundSize = "95%";
document.getElementById('shop_cancel_button').style.opacity = "0%";
if (downloadable.includes('raw') && downloadable.includes('githubusercontent')){
downloadable.replace('https://nuken.xyz/shop/','');
} 
document.getElementById('shop_download_button').innerHTML = "Installing...";
...
var check_content_library_div = function() {
    var libraryDiv = document.getElementById('library_div');
    if (libraryDiv && libraryDiv.innerHTML.trim() == "") {
        document.getElementById('library_div').innerHTML = `
    <button id="bottom_shop_content_teaser" style="    padding: 2.5vmin;
    background-color: lightgray;
    font-size: 3vmin;
    margin-top: 1.5vmin;
    border-radius: 1.5vmin;" onclick = "show_folder('/content/')" >Show content directory</button>
`;
    } 
};

var show_deleted_addon = function(element) {
...
<h3>Removed</h3>
<h4>File deletion will take effect when you reload nuken.</h4>
...
document.getElementById(element).style.transform = "scale(0.99)";
