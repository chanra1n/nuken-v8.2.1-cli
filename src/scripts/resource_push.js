// Because you're looking at this trying to figure what the hell this is and what it does,
// here's a walkthrough on how to define resources for nuken.
...
var javascript_lang = {
	...
	title: "JavaScript",
	// The object name, but because our onclick events don't like it for some reason,
	// we need it in string form. This also has other uses, and it's really handy to have, but we won't get into that here.
	codename: "javascript_lang",
	// The resource icon, written to the correct management menu
	// (we begin looking at index.html's root directory, but a custom url can be used here)
...
`,
	...
	resource: ``,
	// nuken's resources are written to the project BELOW the <body> tags.
	// if the resource needs to be loaded in before anything else, put it here instead of the EXAMPLE.resource.	
	extra: '',
...
`;
	...
	// The image id is the resource.codename + "_icon", and it's given an onclick event: 
	// select_script or select_style. 
	// This actually loads the resource's information into the user's view. So basically, TLDR...

	/*

	1) We create an "resource" object, with the listed properties above.
	2) We push an icon to the menu, and give it an onclick event to fill in the rest of the information when clicked on, 
	based on the "resource" object name.
	3) If the icon is actually clicked, it is "selected" and the title, info bar, description, etc. 
	is filled in based on the properties we've passed to our "resource" object.
...
// This is the aforementioned select function, specifically for the Script Management Menu.
// Yes, technically this is the same as the other select function, I want to keep them seperate just because. Much easier to automate.
...
var select_script = function(resource) {
	...
	// then, we add the "selected_resource" class to the icon we've just clicked on.
	// the "selected_resource" class gives the icon an orange border, full opacity, slightly larger size, etc.
...
// I'm not going to go over this again, it seems pretty self explanatory by this point. 
// Go read everying up there ^^^ if you're not sure about something. 
// It's literally the same function, I'm keeping it seperate because it's nice for third-party devs to be able to populate the menus seperately.
...
// okay imma level with you a lot of the following code is weird but idk, 
// i think i was half asleep while writing it so i'm not going to question why it works. you shouldn't either until we have to replace it. 
...
// push the JS and CSS resource icons to their respective management menus, 
// regardless of whether the user is online or not (since these resources can obviously be used either way)
