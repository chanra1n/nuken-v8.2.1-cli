// Okay, so the notification system is pretty straightforward. 
// I thought the default UIkit notif system was a bit on the complicated side for what we're trying to accomplish,
// so I reskinned it a bit.

// If you want to disable notifications, set this to 'false'.
// This is just so that we can control when notifs appear onscreen (like maybe not showing notifs before the page loads, etc.)

var ready_to_notify = false;

// This is the number of mini-notifications in the mini-nav log area, in the bell icon dropdown.
// When it hits zero (like when the user deletes all of them), the default placeholder graphic fills the empty space.
// In any other case, it disappears to let the mini-notifs fill the space.

var number_of_notifs = 0;


// This is the number badge that appears above the bell icon.
// When the user hovers over it, and enters the menu, this will always be reset to zero,
// (while the number_of_notifs actually IN the menu might remain the same).
...
// The nuken notify function has three parameters: the message, the time it's displayed for (in milliseconds), 
// and the priority of the message.
// You can see more information on the priority parameter (you shouldn't really need it, I kept it in just in case though)
// in the UIkit documentation.

var notify = function(message, time, priority) {
	
    // !!! UIkit.notification(message,{pos: 'bottom-right',timeout:time,status:priority}); !!!

    // get the user's setting from localstorage. 
	// if this is false don't show any notifs at all and overwrite the menu with some placeholder text
...
var project_console_notify = function(message, type) {
    ...
    //the display property value for the "what does this mean?" button that appears under project errors.
	// by default, it isn't seen unless the notification is an error, so we hide it in every other case. 
